...
import { setLoading, setRequestStatus } from "../actions/miscAction";
  ...
  switch (type) {
    ...
    case EDIT_PRODUCT: {
      try {
        ...
        if (imageCollection.length > 1) {
          ...
          //make sure you're adding the url no the file object.
        }
    ...
    case SEARCH_PRODUCT: {
      try {
        ...
        const state = yield select(); //getting state?
        ...
        if (result.products.length === 0) {
          ...
          yield put(
            searchProductSuccess({
              ...
              lastkey: result.lastkey
                ? result.lastkey
                : state.products.searchProducts.lastRefkey,
              total: result.total
                ? result.total
                : state.products.searchProducts.total,
            })
          );
          yield put(setRequestStatus(""));
        }
